...
void PowMiner(CWallet *pwallet)
{
    ...
    // Make this thread recognisable as the mining thread
    RenameThread("Subcriptio-powminer");
    ...
    while (true)
    {
        ...
        while (true)
        {
            if (fShutdown)
                return;
            pblock->nNonce++;
            if (pblock->nNonce > 5000000)
                break;
            if (pblock->nNonce % 1000 == 0 && pindexPrev != pindexBest)
                break;
            if (TryCheckProofOfWork(pblock->GetPoWHash(), pblock->nBits))
            {
